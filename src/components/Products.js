...
// let inpuvalue=
    try {
      ...
      if(e.response.status===500){
        ...
        enqueueSnackbar('Could not fetch products. Check that thr backend is running, reachable and returns valid JSON.',
         { variant: "error" })
      }
    }
    // setLoader(false);
...
if(!event.target.value){return}
   ...
   const addToCart= async(
    ...
    if(!token){
      enqueueSnackbar("Login to add an item to cart",{variant: 'warning'})
      return
    }
  ...
  useEffect(() => {
    ...
    const productsData= performAPICall();
...
onLoadHandler()
  ...
  return (
    <div>
      ...
      />
       <Grid container>
         <Grid item xs={12} md={token && products.length ? 9 : 12} className="product-grid">
           <Box className="hero">
                  ...
                  filteredProducts.map((product)=> (
                    ...
                    <Grid item xs={6} md={3} key={product._id}>
                      ...
                      handleAddToCart={async ()=>{
                        ...
                        addToCart(
                          token,items,products,product._id, 1, {
